...
// Tipos de imóvel para comercial/compra
...
// Itens quantitativos para filtro comercial
const ITENS_COMERCIAL = [
  { chave: "banheiros", label: "Banheiros" },
  { chave: "garagens", label: "Vagas de Garagem" },
  { chave: "metragem", label: "Metragem mínima (m²)" },
];

export default function ComercialComprarPage() {
  ...
  useEffect(() => {
    setImoveisFiltrados(
      filtrarImoveisFrontend(imoveis, filtros, ITENS_COMERCIAL)
    );
  }, [imoveis, filtros]);
      ...
      <main className="flex-1 flex flex-col items-center py-10 px-2">
        <div className="w-full max-w-11/12 mx-auto">
          <div className="text-center mb-5">
            ...
            >
              Imóveis Comerciais para Comprar
            </h1>
            <p
              className="font-inter text-sm md:text-xl text-blue-900"
              style={{ userSelect: "none" }}
            >
              Encontre o imóvel comercial ideal para investir ou expandir seu negócio!
            </p>
          </div>
          ...
          {carregando ? (
            ...
            <div className="font-inter text-sm md:text-lg text-center text-blue-700 bg-blue-100 border border-blue-200 p-6 rounded-xl shadow mt-10">
              Nenhum imóvel comercial para compra encontrado.
            </div>
          ) : (
