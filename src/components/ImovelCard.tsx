...
export default function ImovelCard({ imovel, contexto = "categoria" }: ImovelCardProps) {
  ...
  const itensDisponiveis = useMemo(() => ITENS_POR_SETOR[imovel.tipoNegocio] || [], [imovel.tipoNegocio]);
    ...
    <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 border border-gray-100 flex flex-col">
      ...
      <div className="p-4 flex-1 flex flex-col gap-2">
        ...
        <div className="font-inter text-sm text-gray-600 space-y-1">
          <p><span className="font-poppins font-bold">Local:</span> {formatarTexto(imovel.bairro)}, {formatarTexto(imovel.cidade)}</p>
          <p><span className="font-poppins font-bold">Endereço:</span> {formatarTexto(imovel.enderecoDetalhado)}</p>
          <p><span className="font-poppins font-bold">Área:</span> {imovel.metragem}m²</p>
        </div>
        <button
          ...
          type="button"
        >
          {exibirMais ? <>Ocultar detalhes <FaChevronUp className="text-xs" /></> : <>Exibir mais <FaChevronDown className="text-xs" /></>}
        </button>
        {exibirMais && (
          <div className="mt-2">
            <p className="text-gray-700 text-sm mb-2 whitespace-pre-line">{formatarTexto(imovel.descricao)}</p>
            <div>
                ...
                {itensDisponiveis.map((item) => {
                  ...
                  return (
                    <div key={item.chave} className="flex flex-col items-center bg-blue-50 rounded px-2 py-1 min-w-0">
                      <span className="text-blue-900 text-xs text-center truncate w-full" title={item.label}>{item.label}</span>
                      <span className="text-sm font-semibold text-blue-700 mt-1">{isQuant ? valor : "Sim"}</span>
                    </div>
                  );
                })}
              </div>
